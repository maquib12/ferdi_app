					...
					<div class="col-sm-8">
						<form>
							<div class="d-flex align-items-center mb-4 pb-1">
								...
								<button type="button" class="btn btn-primary rounded-pill px-5" data-toggle="collapse" href="#successful" onClick = "handleNext(event)"><span class="d-block my-1">Next</span></button>
							</div>
					...
					<div class="popup-inner col-sm-9 mx-auto text-center">
						...
						<div class="row w-100 mt-5 ">
							...
							@php
								...
								$moduleData = Module::where(['id'=>$next_mod_id])->count();
								$courseId = $no_of_questions->course_id;
							@endphp
							@if($moduleData>0)
							<div class="col-sm-6 mx-auto">
								<a href="{{route('stu-skill-assessment-completed',['id' => $next_mod_id,'q_id' => 1,'courseId'=>$modules->course_id])}}" class="btn btn-primary btn-block px-5 rounded-md"><span class="d-block my-2">Next Module</span></a>
							</div>
							@else
							<div class="col-sm-6 mx-auto">
								<a href="{{route('stu-skill-assessment-completed',['id' => $next_mod_id,'q_id' => 1,'courseId'=>$modules->course_id])}}" class="btn btn-primary btn-block px-5 rounded-md"><span class="d-block my-2">Next </span></a>
							</div>
							@endif
							@endif
						</div>
...
<script>
	...
	var current_q = path[path.length - 2];
		...
		var total = $('#total').val();
            ...
            error: function (err) {
            	...
            	if(parseInt(current_q)+1 <= total){
                    var path = window.location.origin + `/ferdi_app/public/students/stu-skill-assessment/${module_id}/${id}/${course_id}`;
	...
	function retake(){
		...
		$.ajax({
            ...
            success: function (response) {
            	...
            	window.location.href = window.location.origin + `/ferdi_app/public/students/stu-skill-assessment/`+response.module_id+`/`+response.question_id+`/`+course_id;
            },
            error: function (err) {
            	console.log(" error Set Status");
