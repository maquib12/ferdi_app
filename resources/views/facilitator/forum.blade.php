						...
						<div class="col-sm-6 grid-item">
							<div class="card bg-dark rounded-md px-4 pt-3 mb-4">
								...
								<div class="d-flex justify-content-between border-top border-xlight border-solid-2 py-4">
									...
									<div class="font-12 icon-white" role="button" onclick="forumLike({{$forum->id}})" id="like_btn"><img src="{{asset('assets/img/icons/heart-color.svg')}}" class="icon mr-2">
										<span id="like_{{$forum->id}}">{{isset($totallikes[$forum->id]) ? $totallikes[$forum->id] : 0}}</span> Likes</div>
									<div class="font-12 icon-white" role="button" onclick="forumDislike({{$forum->id}})"id="dislike_btn><img src="{{asset('assets/img/icons/dislike.svg')}}" class="icon mr-2"> <span id="dislike_{{$forum->id}}">{{isset($totaldislikes[$forum->id]) ? $totaldislikes[$forum->id] : 0}}</span> Dislikes</div>
								</div>
								<div class="collapse" id="comment-1{{$forum->id}}">
									<div class="comment-box">
											<div class="add-comment px-1">
												<input type = "hidden" name="forum_id" id="forum_id" value="">
												<input  class="form-control bg-dark font-12 text-white ph-gray pr-5 user_comment{{$forum->id}}" id ="comment_forum2" name="comment" placeholder="Type your Comment Here.....">
												<button class="send" onclick="forumComment({{$forum->id}})" type="button"><img src="{{asset('assets/img/icons/send-2.svg')}}"></button>
											</div>
...
<script>
	function forumComment(forum_id) {
		var comment = $(".user_comment"+forum_id).val();
        $.ajax({
            method: "POST",
            url: "{{ url('facilitator/comments-post') }}",
            headers: {
    ...
    function forumLike(forum_id) {
        ...
        $.ajax({
            ...
            type: "POST",
            dataType: "json",
            url: "{{ url('facilitator/forum_like') }}",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: {
     ...
     function forumDislike(forum_id) {
        ...
        $.ajax({
            ...
            type: "POST",
            dataType: "json",
            url: "{{ url('facilitator/forum_dislike') }}",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: {
