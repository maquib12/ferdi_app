		...
		<section class="py-5">
			<div class="container">
				<div class="heading border-bottom border-xlight border-solid-2 mb-4 pb-1 d-flex align-items-center">
					<h1 class="pb-2 font-bold font-36 my-0 flex-fill">Skill Assessment Edit</h1>
					<div class="action">
					...
					<div class="col-sm-8">
						<form action="{{route('addQuestionsPost')}}" method="POST">
							...
							<div class="d-flex align-items-center mb-4 pb-1">
								...
								@if($question)<input type="hidden" name="question_id" value="{{$question->questions_no}}">@endif
								<button type="submit" class="btn btn-primary rounded-pill px-5"><span class="d-block my-1">Submit</span></button>
							</div>
							@if($question)<textarea class="form-control border-0 lh-32 bg-dark px-4 py-2 rounded-sm resize-none text-white font-xlight shadow-none" name="text" rows="3">{{$question->question}}</textarea>@endif
								...
								<div class="col-sm-6 my-2 py-1">
									<label class="lg-opt d-block">
										...
										<div class="d-flex align-items-center bg-dark p-4 rounded-md">
											...
											<input type="text" class="text form-control border-0 bg-transparent text-white shadow-none" value="{{$question->answer_a}}" name="a">
										</div>
									</label>
								</div>
								<div class="col-sm-6 my-2 py-1">
									<label class="lg-opt d-block">
										...
										<div class="d-flex align-items-center bg-dark p-4 rounded-md">
											...
											<input type="text" class="text form-control border-0 bg-transparent text-white shadow-none" value="{{$question->answer_b}}" name="b">
										</div>
									</label>
								</div>
								<div class="col-sm-6 my-2 py-1">
									<label class="lg-opt d-block">
										...
										<div class="d-flex align-items-center bg-dark p-4 rounded-md">
											...
											<input type="text" class="text form-control border-0 bg-transparent text-white shadow-none" value="{{$question->answer_c}}" name="c">
										</div>
									</label>
								</div>
								<div class="col-sm-6 my-2 py-1">
									<label class="lg-opt d-block">
										...
										<div class="d-flex align-items-center bg-dark p-4 rounded-md">
											...
											<input type="text" class="text form-control border-0 bg-transparent text-white shadow-none" value="{{$question->answer_d}}" name="d">
										</div>
									</label>
								</div>
							</div>
...
<script>
	...
	function openQuestion(id){
		...
		var path = window.location.origin + `/ferdi_app/public/facilitator/questions_edit/${module_id}/${id}`;
