		...
		<section class="py-5">
			<div class="container">
				<div class="heading border-bottom border-xlight border-solid-2 mb-4 pb-1 d-flex align-items-center">
					<h1 class="pb-2 font-bold font-36 my-0 flex-fill">Skill Assessment Add</h1>
					<div class="action">
						<a href="{{url('facilitator/questions_edit',$modules->id)}}" class="ml-3">Bulk Upload</a>
				...
				<div class="row pt-2">
					...
					<div class="col-sm-8">
						<form action="{{route('questionsAddPost')}}" method="POST">
							@csrf
							<div class="d-flex align-items-center mb-4 pb-1">
								<?php $total = $no_of_questions->no_of_question; ?>
								<h4 class="my-0 flex-fill">Question <span id="question_no"></span> /{{$total}}  </h4>
								<input type="hidden" name="total_question" id="total_question" value="{{$total}}">
								<input type="hidden" name="course_id" value="{{$modules->course_id}}">
								<input type="hidden" name="module_id" id="module_id" value="{{$modules->id}}">
								<input type="hidden" name="question_id" id="question_id">
								<button type="submit" class="btn btn-primary rounded-pill px-5"><span class="d-block my-1">Submit</span></button>
							</div>
							@if($question)<textarea class="form-control border-0 lh-32 bg-dark px-4 py-2 rounded-sm resize-none text-white font-xlight shadow-none" name="text" rows="3" readonly>{{$question->question}}</textarea>
							@else
							<textarea class="form-control border-0 lh-32 bg-dark px-4 py-2 rounded-sm resize-none text-white font-xlight shadow-none" name="text" rows="3"></textarea>
							@endif
							@if($question)
							<div class="row mt-3">
							...
							<div class="row mt-3">
								<div class="col-sm-6 my-2 py-1">
									<label class="lg-opt d-block">
										<input type="radio" name="ques-1" value="a" class="edit-opt correct_answer">
										<div class="d-flex align-items-center bg-dark p-4 rounded-md">
											<div class="opt mr-1">A</div>
											<input type="text" class="text form-control border-0 bg-transparent text-white shadow-none" value="" name="a">
										</div>
									</label>
								</div>
								<div class="col-sm-6 my-2 py-1">
									<label class="lg-opt d-block">
										<input type="radio" name="ques-1" value="b" class="edit-opt correct_answer">
										<div class="d-flex align-items-center bg-dark p-4 rounded-md">
											<div class="opt mr-1">B</div>
											<input type="text" class="text form-control border-0 bg-transparent text-white shadow-none" value="" name="b">
										</div>
									</label>
								</div>
								<div class="col-sm-6 my-2 py-1">
									<label class="lg-opt d-block">
										<input type="radio" name="ques-1" value="c" class="edit-opt correct_answer">
										<div class="d-flex align-items-center bg-dark p-4 rounded-md">
											<div class="opt mr-1">C</div>
											<input type="text" class="text form-control border-0 bg-transparent text-white shadow-none" value="" name="c">
										</div>
									</label>
								</div>
								<div class="col-sm-6 my-2 py-1">
									<label class="lg-opt d-block">
										<input type="radio" name="ques-1" value="d" class="edit-opt correct_answer">
										<div class="d-flex align-items-center bg-dark p-4 rounded-md">
											<div class="opt mr-1">D</div>
											<input type="text" class="text form-control border-0 bg-transparent text-white shadow-none" value="" name="d">
										</div>
									</label>
								</div>
							</div>
...
<script>
	...
	function openQuestion(id){
		...
		var path = window.location.origin + `/ferdi_app/public/facilitator/questions_add/${module_id}/${id}`;
