					...
					<div class="col-sm-9 border border-xlight px-3 d-flex flex-column h-100">
						<div class="d-flex align-items-center py-3 border-bottom border-xlight">
							<div class="avatar-v md mr-3">
								<img src="" class="img-fit rounded-xs d-none" id='stu_image'>
							</div>
							<div class="flex-fill">
							...
							<div>
								...
								<a onclick="chatDelete({{$mentor[0]['id']}})" class="text-white ml-4">
						...
						<!-- <form action="{{route('chatPost')}}" id="chatPost" method="post"> -->
							<div class="chat-footer pb-4 d-flex">
								...
								<div class="bg-dark border-0 rounded-pill d-flex flex-fill">
									<input type="text" class="shadow-none border-0 flex-fill bg-transparent px-4 input text-white" placeholder="Enter your message here," name="text" id="text">
	...
	function openChat(curr, id,name) 
		{
			let message = $('#text').val();
				...
				var receiverImage = $("#stu_image").attr('src');
...
function openTab(id,name,image_user){
	$("#stu_image").removeClass('d-none');
    $('#reciever_id').val(id);
	$('#reciever_name').val(name);
	$('#chatName').html(name);
	$("#stu_image").attr("src",image_user);
